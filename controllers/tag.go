...
func EditTag(c *gin.Context) {
	...
	tags := models.Tag{}
	users := []models.User{}
	config.DB.Where("tags.id = ?", id).Find(&tags)
	session := sessions.Default(c)
	user := session.Get("user")
	var userId int
...
func UpdateTag(c *gin.Context) {
	...
	tags := []models.Tag{}
	config.DB.Model(&tags).Where("id = ?", tagid).Update("name", name)
	session.Save()
